...
from eval_metrics import compute_metric_scores, compute_semantic_metric_scores
...
def parse_args():
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        default='/local/scratch0/data_and_checkpoint/models',
        help=(
            "path to save model"
        ),
    )
    ...
    parser.add_argument(
        "--train_data_path",
        type=str,
        default='/users/PAS2473/brucewan666/ECG/ECG/instruct_data/ptbxl_ecg_train.jsonl',
        help=(
            "train_data_path"
        ),
    )
    ...
    parser.add_argument(
        "--val_data_path",
        type=str,
        default='/users/PAS2473/brucewan666/ECG/ECG/instruct_data/ptbxl_ecg_val.jsonl',
        help=(
            "val_data_path"
        ),
    )
    ...
    parser.add_argument(
        "--test_data_path",
        type=str,
        default='/users/PAS2473/brucewan666/ECG/ECG/instruct_data/ptbxl_ecg_test.jsonl',
        help=(
            "test_data_path"
        ),
    )
    ...
    def _concat_messages(messages):
        ...
        for message in messages:
            if message["role"] == "system":
                ...
                prompt = "generate a description of the ECG embeddings: "
...
def save_with_accelerate(accelerator, model, tokenizer, output_dir, args):
    llama_name_list = []
    unwrapped_model = accelerator.unwrap_model(model.llm_model)
...
def save_with_accelerate_test(accelerator, model, tokenizer, output_dir, args):
    ...
    if args.use_lora:
        ...
        if hasattr(llm_model, "pretrained_model"): # for models with valuehead
            backbone_model = getattr(llm_model, "pretrained_model")
        else:
            backbone_model = llm_model
        # When using lora, the unwrapped model is a PeftModel, which doesn't support the is_main_process 
        # and has its own save_pretrained function for only saving lora modules.
        # We have to manually specify the is_main_process outside the save_pretrained function.
        if accelerator.is_main_process:
        ...
        unwrapped_model = accelerator.unwrap_model(model.module.llm_model)
        unwrapped_model_ecg = accelerator.unwrap_model(model.module.ecg_encoder)
        state_dict = accelerator.get_state_dict(model.module.llm_model)
        state_dict_ecg = accelerator.get_state_dict(model.module.ecg_encoder)
...
def main():
    ...
    if args.train_data_path is not None:
        data_files["val"] = args.val_data_path
    ...
    if args.tokenizer_name:
        if args.llm_type == 'gpt_neox':

            tokenizer = GPTNeoXTokenizerFast.from_pretrained(args.tokenizer_name, use_fast=not args.use_slow_tokenizer)
        else:

            tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name, use_fast=not args.use_slow_tokenizer)

        if args.llm_type == 'bloom':
    ...
    if args.use_ecg_llm:
        ...
        if isinstance(tokenizer, GPT2Tokenizer) and isinstance(model, OPTForCausalLM):
    ...
    with accelerator.main_process_first():  
        train_lm_datasets = raw_datasets.map(
            encode_function,
            batched=False,
            num_proc=args.preprocessing_num_workers,                  ##################### modified test here for bug #################
            load_from_cache_file=not args.overwrite_cache,         
            remove_columns=[name for name in raw_datasets["train"].column_names if name not in ["input_ids", "labels", "attention_mask", "ecg"]],
    ...
    with accelerator.main_process_first():  
        test_lm_datasets = raw_datasets.map(
            encode_function,
            batched=False,
            num_proc=args.preprocessing_num_workers,
            load_from_cache_file=not args.overwrite_cache,
            remove_columns=[name for name in raw_datasets["test"].column_names if name not in ["input_ids", "labels", "attention_mask", "ecg"]],
            desc="Tokenizing and reformatting instruction data",
        )
    ...
    with accelerator.main_process_first():  
        val_lm_datasets = raw_datasets.map(
            encode_function,
            batched=False,
            num_proc=args.preprocessing_num_workers,
            load_from_cache_file=not args.overwrite_cache,
            remove_columns=[name for name in raw_datasets["val"].column_names if name not in ["input_ids", "labels", "attention_mask", "ecg"]],
            desc="Tokenizing and reformatting instruction data",
        )
        val_lm_datasets.set_format(type="pt")
        val_lm_datasets = val_lm_datasets.filter(lambda example: (example['labels'] != -100).any())
    ...
    test_dataset = test_lm_datasets["test"]
    val_dataset = val_lm_datasets["val"]
    ...
    train_dataloader = DataLoader(
        train_dataset, 
        shuffle=True, 
        collate_fn=DataCollatorForSeq2Seq(tokenizer=tokenizer, model=model.llm_model, padding="longest", ecg_data_name = args.ecg_data_name), ########## modified here ###############
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if step > 9 and  step % args.test_step == 0:
                ...
                if accelerator.is_main_process:
                    with torch.no_grad():
                        for num_idx, batch in enumerate(tqdm(test_dataloader)):
                            ...
                            if args.llm_type == 'bloomxxx':  # left padding for bloom

                                for i in range(input_ids_num):

                                    origin_input = input_ids[i, :first_non_minus_100[i]]

                                    padding = torch.tensor((input_ids_len - len(origin_input)) * [3]).to(input_ids.dtype).to(input_ids.device)

                                    if padding is not None:
                                   
                                        new_input = torch.concat([padding, origin_input])
                                    else:
                                        new_input = origin_input

                                    padding_list.append(new_input)

                                input_ids_for_inference = torch.stack(padding_list).to(input_ids.device)
                            ...
                            if args.llm_type == 'opt':
                                ...
                                generate_reports = [ sample.replace(prompt_text, "").replace('<pad>', '').strip() for sample in generate_reports]

                            else:

                                generate_reports = [ sample.replace(prompt_text, "") for sample in generate_reports]

                            reference_reports = [ sample.replace(prompt_text, "") for sample in reference_reports]
                            ...
                            language_model_scores = compute_metric_scores(nlg_metrics, generate_reports, reference_reports, raw_datasets['test'])
