...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, ecg_emb=None, attention_mask=None, head_mask=None):
        ...
        # query torch.Size([16, 20, 127, 64])
        # torch.Size([16, 20, 127, 64])
        # torch.Size([16, 20, 127, 64])

        attn_weights = torch.matmul(query, key.transpose(-1, -2)) # torch.Size([16, 20, 127, 127])
        ...
        if ecg_emb is not None: # ecg_emb = (batch, 128) -> (batch, ec, 128)

            # breakpoint()

            ecg_emb = ecg_emb.to(query.dtype)

            ecg_hidden_states = torch.repeat_interleave(ecg_emb.unsqueeze(1), self.num_heads, dim=1)
            ecg_hidden_states = ecg_hidden_states.unsqueeze(1) # torch.Size([16, 20, 128])
            bsz = query.shape[0]
            ...
            _, ecg_key_states, ecg_value_states = self.c_attn(ecg_hidden_states).split(self.split_size, dim=2)

            ecg_key_states = self._split_heads(ecg_key_states, self.num_heads, self.head_dim)
            ecg_value_states = self._split_heads(ecg_value_states, self.num_heads, self.head_dim)
            ...
            ecg_attn_weights = torch.matmul(query, ecg_key_states.transpose(-1, -2).to(query.dtype))

            if self.scale_attn_weights:
                ecg_attn_weights = ecg_attn_weights / torch.full(
                [], value.size(-1) ** 0.5, dtype=ecg_attn_weights.dtype, device=ecg_attn_weights.device
            )

            attn_weights =  torch.concat([ecg_attn_weights, attn_weights], dim=-1)
